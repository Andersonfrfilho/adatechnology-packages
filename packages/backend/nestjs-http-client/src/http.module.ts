...
import { CACHE_PROVIDER, InMemoryCacheProvider, RedisCacheProvider } from '@adatechnology/nestjs-cache'
...
export class HttpModule {
   ...
   */
  static forRoot(configOrOptions: any = {}, options: HttpModuleOptions = {}): DynamicModule {
    ...
    const providers: Provider[] = this.createCacheProviders(configOrOptions, options, cacheToken)
  ...
  static forRootAsync(options: {
    ...
    useFactory: (
      ...args: any[]
    ) => Promise<{ config: any; options: HttpModuleOptions }> | { config: any; options: HttpModuleOptions }
    inject?: any[]
    provide?: any
    cacheToken?: any
  }): DynamicModule {
    ...
    return {
      ...
      providers: [
        ...
        {
          provide: cacheToken,
          useFactory: async (moduleOptions: { config: any; options: HttpModuleOptions }) => {
            const { config, options: httpOptions } = moduleOptions
            const shouldUseCache = config.useCache !== false && httpOptions.useCache !== false
        ...
        {
          provide: httpToken,
          useFactory: (moduleOptions: { config: any; options: HttpModuleOptions }, cache?: any, logger?: any) => {
            const { config, options: httpOptions } = moduleOptions
            const isAxiosConfig = config.baseURL || config.timeout || config.headers
            const axiosConfig = isAxiosConfig ? config : undefined
            const axiosInstance = axiosConfig ? axios.create(axiosConfig) : undefined
            return new AxiosHttpProvider(axiosInstance, httpOptions, cache, logger)
          },
          inject: [
            'HTTP_MODULE_OPTIONS',
            { token: cacheToken, optional: true },
  ...
  private static createCacheProviders(configOrOptions: any, options: HttpModuleOptions, cacheToken: any): Provider[] {
    ...
    if (shouldUseCache) {
      if (options.cacheInstance !== undefined) {
        ...
        const looksLikeProvider = maybeProvider && (maybeProvider as any).provide
        ...
        // Capturado fora da factory: o narrowing de `options.cache?.` não sobrevive ao closure,
        // que só roda quando o Nest resolve o provider.
        const redisOptions = options.cache.redisOptions
        providers.push({
          provide: cacheToken,
          useFactory: () => new RedisCacheProvider(redisOptions),
        })
      } else {
        providers.push({
          provide: cacheToken,
