...
export type MountModuleRoutesParams = {
  readonly app: UwsApp
...
function writeStream(params: {
  response: UwsResponse
  result: Extract<HttpResult, { kind: 'stream' }>
  isAborted: () => boolean
}): void {
...
function writeResult(params: { response: UwsResponse; result: HttpResult; isAborted: () => boolean }): void {
  const { response, result } = params
  if (params.isAborted()) return
...
export function mountModuleRoutes(params: MountModuleRoutesParams): void {
