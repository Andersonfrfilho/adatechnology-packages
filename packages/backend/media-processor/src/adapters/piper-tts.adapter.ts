...
export class PiperTTSAdapter {
  ...
  > {
    const results = await Promise.all(
      segments.map((seg, i) => this.generateSpeech(seg.text, languageCode, `${outputDir}/segment-${i}.wav`)),
    )

    return results.map(({ audioPath, duration }) => ({ segmentPath: audioPath, duration }))
  }
