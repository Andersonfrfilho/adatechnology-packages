...
import { MediaJob, MediaJobInput, MediaJobStatus, ProcessingStage } from '../types/index.js'
...
export class MediaJobService {
  ...
  constructor(
    ...
    this.logger = logger
    this.queue = new Queue(QUEUE_CONFIG.QUEUE_NAME, { connection })
  }
  ...
  async markCompleted(jobId: string, result: Record<string, any>): Promise<void> {
    ...
    const duration = job.startedAt ? Math.round((Date.now() - job.startedAt.getTime()) / 1000) : undefined
  ...
  async getJobsByStatus(status: MediaJobStatus): Promise<MediaJob[]> {
    // JobRepository ainda não expõe consulta por status — sem isso não há como implementar.
    void status
    return []
  }
  ...
  async getJobMetrics(): Promise<{
    ...
    const jobCounts = await this.queue.getJobCounts('waiting', 'active', 'completed', 'failed')

    return {
      totalPending: jobCounts.waiting ?? 0,
      totalProcessing: jobCounts.active ?? 0,
