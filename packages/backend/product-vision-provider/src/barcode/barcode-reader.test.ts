...
// Os `typeName` sao os que o zbar realmente devolve. Escrever nome "bonito" aqui e o que
// mascarou o defeito por uma release inteira: o dublê concordava com a invencao.
...
describe('leitura de codigo de barras', () => {
  it('devolve o EAN-13 encontrado', async () => {
    const reading = await readerWith([{ typeName: 'ZBAR_EAN13', value: '7891000100103' }]).read(JPEG)
  ...
  it('QR Code na gondola nao vira codigo de produto', async () => {
    ...
    const reading = await readerWith([{ typeName: 'ZBAR_QRCODE', value: 'https://promo.exemplo' }]).read(JPEG)
  ...
  it('simbolo sem digito nao passa por GTIN', async () => {
    ...
    const reading = await readerWith([{ typeName: 'ZBAR_CODE128', value: 'LOTE-ABC-2026' }]).read(JPEG)
  ...
  it('mime nao suportado nao contribui, e nao derruba a cadeia', async () => {
    const reading = await readerWith([{ typeName: 'ZBAR_EAN13', value: '7891000100103' }]).read({
      ...JPEG,
