...
export type EdgeGrid = {
  ...
  readonly columnStartsFor: (input: { readonly fromM: number; readonly sizeM: number }) => readonly number[]
}
...
export function createEdgeGrid(input: {
   ...
   */
  const startsFor = (edges: readonly number[], fromM: number, sizeM: number, toM: number): number[] => {
    const starts: number[] = []
