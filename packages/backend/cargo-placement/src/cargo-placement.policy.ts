 ...
 */
import type { LoadingAccess } from './loading-access.constant.js'
...
export const PLACEMENT_REASONS = [
  ...
  'outOfReach',
  'needsRehandling',
  /** Spec 148 D5: a caixa da passada final que ficou por cima de entrega anterior — ver `coversStops`. */
  'overEarlierDelivery',
] as const
...
export type PlacedBox = {
  ...
  readonly zM: number
  /**
   * Spec 148 D5: as entregas anteriores que esta caixa cobre — quem descarrega tira ela do caminho na
   * primeira delas. Só na caixa da passada final (`overEarlierDelivery`).
   */
  readonly coversStops?: readonly number[]
}

export type UnplacedBox = {
  readonly count: number
  /**
   * Spec 148 (T7): a nota da caixa que ficou de fora — uma linha por nota. Ausente em planta antiga e
   * quando a caixa não tem nota — aí a linha é a do formato antigo.
   */
  readonly documentId?: string | null
  readonly label: string
 ...
 */
export function resolveCargoPlacement(input: Parameters<typeof placeCargo>[0]): CargoPlacement | null {
...
function areTouching(first: BoxExtent, second: BoxExtent): boolean {
  ...
  return axes.some((axis, index) => {
    ...
    return axes.every((other, otherIndex) => {
      ...
      return Math.min(fromC + sizeC, fromD + sizeD) - Math.max(fromC, fromD) > NOTE_CONTACT_OVERLAP_M
    })
  })
}
...
function placeCargoOnce(
  ...
  const measured = input.boxes.filter((box) => {
    ...
    unplaced.push({ count: box.count, ...unplacedNoteOf(box), label: box.label, reason: 'notMeasured' })
    return false
  })
  ...
  const laneSlackM = !lanes
    ...
    : Math.max(
        ...
        packBed.lengthM -
          sequences.reduce(
            (total, stopSequence) =>
              total + minimumLaneWidthOf(measured.filter((box) => box.stopSequence === stopSequence)),
            0,
          ),
      )
  ...
  const freeM = Math.max(0, packBed.lengthM - slices.reduce((total, slice) => total + slice.lengthM, 0))
  ...
  for (const slice of slices) {
    ...
    for (const box of slice.leftovers) leftovers.push({ box, sliceSizeM: slice.lengthM, sliceStartM })
...
function packUntilItFits(input: {
   ...
   */
  const floorM = crossSectionM2 > 0 ? volumeOf(input.boxes) / crossSectionM2 / ROW_PACKING_EFFICIENCY : input.capM
  /**
   ...
   */
  let lengthM = input.stackBeforeRow === true ? input.capM : Math.min(input.capM, Math.max(floorM, deepestM))
...
function usableStackHeightOf(boxes: readonly PlacementBox[]): number {
  return Math.max(
    ...boxes.map(
      (box) => (Math.min(box.lengthMm ?? 0, box.widthMm ?? 0) / MILLIMETRES_PER_METRE) * STABLE_STACK_SLENDERNESS,
    ),
  )
}
...
function minimumLaneWidthOf(boxes: readonly PlacementBox[]): number {
  return Math.max(...boxes.map((box) => Math.min(box.lengthMm ?? 0, box.widthMm ?? 0) / MILLIMETRES_PER_METRE))
}
...
function volumeOf(boxes: readonly PlacementBox[]): number {
  return boxes.reduce(
    (total, box) =>
      total + (box.count * (box.lengthMm ?? 0) * (box.widthMm ?? 0) * (box.heightMm ?? 0)) / MILLIMETRES_PER_METRE ** 3,
    0,
  )
}
...
function packSlice(input: {
   ...
   */
  const seatAttempts = Math.max(MAX_SEAT_ATTEMPTS, SEAT_ATTEMPTS_PER_ROW * Math.ceil(slice.widthM / SEAT_BUDGET_STEP_M))
  /**
   * Spec 120: quem pousa em quem, para o complemento nunca prender uma caixa recomendada da própria
   * entrega embaixo dele.
   */
  const occupancy =
    input.complement === true ? createOccupancyGrid({ lengthM: slice.lengthM, widthM: slice.widthM }) : null
  ...
  outer: for (const box of ordered) {
    ...
    for (let unit = 0; unit < box.count; unit += 1) {
      ...
      if (placed.length >= input.budget) {
        pushUnplaced(unplaced, { count: 1, ...unplacedNoteOf(box), label: box.label, reason: 'tooMany' })
        continue
      }
      /**
      ...
      const placeAt = (seat: SeatCandidate & { readonly yM: number }, isComplement: boolean): PlacedBox => {
      ...
      if (pending !== undefined && pending.at === placed.length && pending.stopSequence === box.stopSequence) {
        ...
        const rows = [...new Set([...pending.rows, ...nearRows])].sort((first, second) => first - second)
      ...
      while (input.wallBuilding !== true && guard < seatAttempts) {
        ...
        const rejectEarly =
          input.deliveryOrder === true
            ? (xM: number): boolean =>
                (occupancy === null ||
                  ((reachFallback as SeatCandidate | null) !== null && reachRows.at(-1) === rowYM)) &&
...
export function createSupportMap(
  ...
  readonly rows: (input: { readonly fromM: number; readonly toM: number; readonly widthM: number }) => readonly number[]
  readonly seat: (input: {
   ...
   */
  const braceSides: Readonly<Record<'columnEnd' | 'columnStart' | 'lineEnd' | 'lineStart', BraceSide>> = {
    columnEnd: {
      forward: true,
  ...
  const bracedToward = (side: BraceSide, across: number, faceM: number): boolean => {
    ...
    for (;;) {
      ...
      if (outside) {
        return openFace === 'lineEnd' && !side.forward && !side.isColumn ? leansOnHeadboard(faceM) : side.holds
      }
  ...
  return {
    ...
    seat: ({ accept, heightM, onlyOverLoad, rejectEarly, rejectShadowed, slot, xM, yM }) => {
      ...
      const shadowLine = rejectShadowed === true && faceM < bed.widthM - EDGE_TOLERANCE_M ? grid.lineAt(faceM) : -1
      const shadows: number[] = []
      let shadowsHead = 0
      const shadowOf = (column: number): number => (shadowLine < 0 ? 0 : (laterFrontM[column]?.[shadowLine] ?? 0))
      ...
      for (const x of grid.columnStartsFor({ fromM: xM, sizeM: slot.depthM })) {
        ...
        for (; pushed < end; pushed += 1) {
          ...
          while (highest.length > highestHead && (bandHigh[highest[highest.length - 1] ?? 0] ?? 0) <= high) {
            highest.pop()
          }
          highest.push(pushed)
          while (
          ...
          if (shadowLine >= 0) {
            ...
            while (shadows.length > shadowsHead && shadowOf(shadows[shadows.length - 1] ?? 0) <= shade) {
              shadows.pop()
            }
            shadows.push(pushed)
          }
...
function placeSplitCargo(input: {
  ...
  const queue = [...input.leftovers].sort((first, second) => first.box.stopSequence - second.box.stopSequence)
  ...
  for (const { box, sliceSizeM, sliceStartM } of queue) {
    ...
    const slot = fitSlot({ bed: input.bed, box })
    const shapeKey =
      slot === null ? '' : `${box.stopSequence}|${slot.depthM}|${slot.widthM}|${slot.heightM}|${sliceStartM}`
    const spot =
...
function createDeadSpaceTracker(input: {
  ...
  const isSmall = (slot: Slot): boolean =>
    dominant !== null &&
    (cellAreaOf(slot) < cellAreaOf(dominant) || (isOffPattern(slot) && cellAreaOf(slot) <= cellAreaOf(dominant)))
  /**
  ...
  const findInBand = ({ accept, frontierM, onlyOverLoad, slot, support }: Search, dominantSlot: Slot) => {
    ...
    for (const yM of support.rows({
      ...
      const found = support.seat({
        accept: (candidate) =>
          input.slice.heightM - (candidate.topM + slot.heightM) < dominantSlot.heightM - 1e-9 && acceptRow(candidate),
...
function resolveDominantSlot(input: {
  ...
  return dominant === null ? null : fitSlot({ bed: input.slice, box: dominant.box, deepAxis: input.deepAxis })
}
...
function fitSlot(input: {
  ...
  const usable = orientations.filter((slot) => slot.depthM <= input.bed.lengthM && slot.widthM <= input.bed.widthM)
  ...
  const acrossOf = (spanM: number, sizeM: number): number => Math.floor((spanM - sizeM + 1e-9) / cellSizeOf(sizeM)) + 1
...
/** Spec 148 (T7): a nota da caixa que ficou de fora — só quando a caixa tem nota, para o formato antigo não mudar. */
function unplacedNoteOf(box: PlacementBox): { readonly documentId?: string } {
  return box.documentId === null || box.documentId === undefined ? {} : { documentId: box.documentId }
}

/**
 * Uma linha por motivo, por rótulo e por nota: dez caixas iguais fora não viram dez avisos, e duas notas da
 * mesma parada não viram uma (spec 148, T7).
 */
function pushUnplaced(list: UnplacedBox[], entry: UnplacedBox): void {
  const existing = list.find(
    (item) =>
      ...
      (item.documentId ?? null) === (entry.documentId ?? null),
  )
...
export function resolveGridLanes(
  ...
  const sequences = [...new Set(measured.map((box) => box.stopSequence))].sort((first, second) => first - second)
  if (sequences.length < 2) return null
  const widestM = Math.max(
    ...sequences.map((stopSequence) => minimumLaneWidthOf(measured.filter((box) => box.stopSequence === stopSequence))),
  )
  ...
  for (let laneCount = most; laneCount >= 2; laneCount -= 1) {
    ...
    const laneOf = new Map(sequences.map((stopSequence, index) => [stopSequence, index % laneCount]))
    const fits = Array.from({ length: laneCount }, (_, lane) => lane).every((lane) => {
       ...
       */
      return volumeOf(own) <= laneWidthM * input.bedLengthM * input.bedHeightM * ROW_PACKING_EFFICIENCY
    })
...
function gridOrDepth(
  ...
  for (
    ...
    grid !== null;
    grid = grid.laneCount > 2 ? resolveGridLanes({ ...dimensions, maxLanes: grid.laneCount - 1 }) : null
  ) {
...
function placeDeliveryBlock(input: {
  ...
  const { packed } = best
  /**
   * ⚠️ **Spec 148 D5/D6: a passada final — só no baú fechado com complemento, e só sobre a tentativa que
   * venceu.** O que nem a varredura nem o complemento colocaram tenta os vãos que sobraram, inclusive por
  ...
  const over =
    input.enclosedBody && input.complement && best.complement.rejected.length > 0
      ? placeOverEarlierDeliveries({
          ...
          placed: [...packed.boxes, ...best.complement.boxes],
          reachM: input.reachM,
...
function placeComplement(input: {
  ...
  const sequences = [...new Set([...input.placed, ...input.overflow].map((box) => box.stopSequence))].sort(
 ...
 * colocaram tenta os vãos que sobraram, em qualquer orientação que `keepUpright` permita, e fica o assento
 * mais alto — os vãos altos primeiro. É a única passada que fura a ordem de descarga: pode ficar por cima de
 ...
 */
export function placeOverEarlierDeliveries(input: {
  readonly bed: Readonly<{ heightM: number; lengthM: number; widthM: number }>
  readonly enclosedBody: boolean
  readonly openSides?: OpenSides
  readonly placed: readonly PlacedBox[]
  ...
  const support = createSupportMap(input.bed, 'lineEnd', input.openSides ?? CLOSED_SIDES, input.reachM)
  /** O relevo é carimbado de baixo para cima: toda caixa pousa acima do que já está na pegada. */
...
function singleBoxVolumeOf(box: PlacementBox): number {
  return (box.lengthMm ?? 0) * (box.widthMm ?? 0) * (box.heightMm ?? 0)
}

/** As orientações da caixa: as duas de pé sempre; deitada e de lado só se `keepUpright` não proíbe. */
function orientationsOf(
  box: PlacementBox,
  bed: Readonly<{ heightM: number; lengthM: number; widthM: number }>,
): Slot[] {
  const [lengthM, widthM, heightM] = [box.lengthMm, box.widthMm, box.heightMm].map((sizeMm) =>
    toMillimetreSize((sizeMm ?? 0) / MILLIMETRES_PER_METRE),
  ) as [number, number, number]
  const upright: [number, number, number][] = [
    [lengthM, widthM, heightM],
    [widthM, lengthM, heightM],
  ]
  const lying: [number, number, number][] =
    box.keepUpright === true
      ? []
      : [
          [lengthM, heightM, widthM],
          [heightM, lengthM, widthM],
          [widthM, heightM, lengthM],
          [heightM, widthM, lengthM],
        ]
  const seen = new Set<string>()
  const slots: Slot[] = []
  for (const [depthM, footWidthM, slotHeightM] of [...upright, ...lying]) {
    const key = `${String(depthM)}|${String(footWidthM)}|${String(slotHeightM)}`
    if (seen.has(key)) continue
    seen.add(key)
    if (depthM > bed.lengthM + 1e-9 || footWidthM > bed.widthM + 1e-9 || slotHeightM > bed.heightM + 1e-9) continue
    slots.push({ depthM, heightM: slotHeightM, widthM: footWidthM })
  }
  return slots
}

type OverSeat = Readonly<{ coversStops: readonly number[]; slot: Slot; topM: number; xM: number; yM: number }>

/** O assento mais alto que as regras aceitam, em qualquer orientação — ver `placeOverEarlierDeliveries`. */
function findOverSeat(input: {
  readonly box: PlacementBox
  readonly cargo: readonly PlacedBox[]
  ...
  const { box, cargo, pass, support } = input
  const { bed } = pass
  let best: OverSeat | null = null
  for (const slot of orientationsOf(box, bed)) {
    ...
    for (const yM of support.rows({ fromM: 0, toM: bed.widthM - slot.widthM, widthM: slot.widthM })) {
      ...
      const found = support.seat({
        accept: ({ topM, xM }) => {
          if (xM + slot.depthM > bed.lengthM + 1e-9) return false
          if (best !== null && topM < best.topM - 1e-9) return false
          const at = { slot, topM, xM, yM }
          const standing = isStandingUp({
            isRestrainedUpTo: (restraintM) =>
              support.isConfined({ ...at, baseM: topM, enclosed: pass.enclosedBody, topM: restraintM }),
          ...
          if (!standing) return false
          const covered = overlookOf({ at, cargo, stopSequence: box.stopSequence })
          if (covered === null) return false
          const firstStop = Math.min(box.stopSequence, ...covered)
          if (!isWithinReachAt({ at, bed, cargo, fromStop: firstStop, reachM: pass.reachM })) return false
          coversStops = covered
          return true
        },
        heightM: bed.heightM,
      ...
      if (
        best === null ||
        found.topM > best.topM + 1e-9 ||
        (Math.abs(found.topM - best.topM) <= 1e-9 && yM > best.yM)
      ) {
        best = { coversStops, slot, topM: found.topM, xM: found.xM, yM }
      }
    }
    if (!foundAny) input.failedAt.set(shapeKey, input.version)
  }
  return best
}
...
function overlookOf(input: {
  ...
  for (const other of input.cargo) {
    const overlapsX = other.xM < toXM - EDGE_TOLERANCE_M && other.xM + other.depthM > at.xM + EDGE_TOLERANCE_M
    if (!overlapsX) continue
    const otherTopM = other.zM + other.heightM
    const overlapsY = other.yM < toYM - EDGE_TOLERANCE_M && other.yM + other.widthM > at.yM + EDGE_TOLERANCE_M
    if (overlapsY && otherTopM <= at.topM + EDGE_TOLERANCE_M) {
...
/**
 * O alcance da mão (D24) na hora em que a caixa sai do caminho: quando a entrega `fromStop` desce, quem
 * descarrega fica de pé no piso livre, na frente da carga das entregas `≥ fromStop`, pelo corredor mais raso
 * que encosta na caixa — a mesma leitura de `isOutOfReach`.
 */
function isWithinReachAt(input: {
  readonly at: Readonly<{ slot: Slot; topM: number; xM: number; yM: number }>
  readonly bed: Readonly<{ heightM: number; lengthM: number; widthM: number }>
  readonly cargo: readonly PlacedBox[]
  ...
  const { at, bed } = input
  const floor = input.cargo.filter((box) => box.zM < EDGE_TOLERANCE_M && box.stopSequence >= input.fromStop)
  const corridorM = Math.min(ACCESS_CORRIDOR_M, bed.lengthM)
  const lowM = Math.max(0, at.xM - corridorM + 2 * EDGE_TOLERANCE_M)
  const highM = Math.max(lowM, Math.min(bed.lengthM - corridorM, at.xM + at.slot.depthM - 2 * EDGE_TOLERANCE_M))
  let standingM = Number.POSITIVE_INFINITY
  for (let startM = lowM; startM <= highM + 1e-9; startM += 0.05) {
    const endM = startM + corridorM
    const deepestM = floor.reduce(
      (deepest, box) =>
        box.xM < endM - EDGE_TOLERANCE_M && box.xM + box.depthM > startM + EDGE_TOLERANCE_M
          ? Math.max(deepest, box.yM + box.widthM)
          : deepest,
      0,
    )
    standingM = Math.min(standingM, deepestM)
  }
  if (standingM === Number.POSITIVE_INFINITY) return true

  return standingM - (at.yM + at.slot.widthM) <= input.reachM + 1e-9
}
    ...
    isRestable: ({ slot, stopSequence, topM: base, xM, yM }) => {
      ...
      for (let column = fromColumn; column < toColumn; column += 1) {
        for (let line = fromLine; line < toLine; line += 1) {
          ...
          if ((ownerIsComplement[column]?.[line] ?? 0) !== 1 && (ownerSequence[column]?.[line] ?? 0) <= stopSequence) {
            return false
          }
        }
      }
