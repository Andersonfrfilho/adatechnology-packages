 ...
 */
import type { PlacementBox } from '../cargo-placement.policy.js'
...
export function createMixedLoad(input: MixedLoadInput): MixedLoad {
  ...
  const random = createRandom(input.seed * 7919 + input.shapeCount * 131 + Math.round(input.occupancy * 100))
  ...
  const bedVolumeM3 = Number(input.bed.lengthM) * Number(input.bed.widthM) * Number(input.bed.heightM)
