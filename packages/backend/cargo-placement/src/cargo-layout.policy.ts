 ...
 */
import type { CargoEstimateSource } from './cargo-estimate-source.types.js'
...
import { divideHalfUp, formatScaledDecimal, parseScaledDecimal } from './decimal.service.js'
...
export function resolveCargoLayout(input: {
  ...
  const section = bed === null ? 0n : divideHalfUp(toLength(bed.widthM) * toLength(bed.heightM), LENGTH_FACTOR)
  ...
  for (const [index, stop] of ordered.entries()) {
    depthByLoadOrder.set(
      ...
      bedKnown ? divideHalfUp(toScaled(stop.volumeM3) * LENGTH_FACTOR, section * VOLUME_TO_LENGTH_FACTOR) : 0n,
    )
  }
  ...
  const cargoRows =
    !occupancyKnown || loaded >= capacity
      ? rowCount
      : Math.min(rowCount, Math.max(ordered.length, Math.round((Number(loaded) / Number(capacity)) * rowCount)))
  ...
  return {
    ...
    loadingAccess: access,
    placement,
    bedWidthM: bed === null || !bedKnown ? null : formatScaledDecimal(toLength(bed.widthM), LENGTH_SCALE),
    freeDepthM: bedKnown
    ...
    overflowM3: formatScaledDecimal(occupancyKnown && loaded > capacity ? loaded - capacity : 0n, VOLUME_SCALE),
    ...
    for (const box of stop.boxes ?? []) {
      ...
      if (current === undefined) {
        grouped.set(key, {
          ...
          documentNumber,
          estimateSource: box.estimateSource ?? (input.fallbackBoxVolumeM3 !== null ? 'median' : 'none'),
...
function toPlacementBoxes(
  ...
  return input.stops.flatMap((stop) =>
    (stop.boxes ?? []).map((box) => {
       ...
       */
      const fallback = estimatedVolumeM3 === null ? companyFallback : resolveFallbackBoxForVolume(estimatedVolumeM3)
...
function buildLayoutNotes(input: {
  ...
  const limitM = Math.min(...boxes.map((box) => Math.min(box.depthM, box.widthM) * STABLE_STACK_SLENDERNESS))
