...
import { resolveStopArrangement, type PlacementBox } from '../../src/cargo-placement.policy.js'
...
describe('qual arranjo a viagem usa (spec 100 D2)', () => {
  ...
  test('a metade exata ainda é faixa', () => {
    expect(resolveStopArrangement({ bed: FIORINO, boxes: tresParadas(), payloadRatio: '0.5000' })).toMatchObject({
      arrangement: 'lanes',
    })
  })
  ...
  test('teto de massa desconhecido não derruba as faixas', () => {
    expect(resolveStopArrangement({ bed: FIORINO, boxes: tresParadas(), payloadRatio: null })).toMatchObject({
      arrangement: 'lanes',
    })
  })
  ...
  test('sem as medidas do baú o arranjo é profundidade', () => {
    expect(resolveStopArrangement({ bed: null, boxes: tresParadas(), payloadRatio: null })).toMatchObject({
      arrangement: 'depth',
      reason: 'noBed',
    })
  })
  ...
  test('a parada menor não derruba as faixas só por ser pequena', () => {
    const arranjo = resolveStopArrangement({
      bed: FIORINO,
      boxes: [box({ count: 12, stopSequence: 1 }), box({ stopSequence: 2 }), box({ stopSequence: 3 })],
      payloadRatio: null,
    })
  ...
  test('a parada dominante estrangulada pelo mínimo das vizinhas derruba as faixas', () => {
    const arranjo = resolveStopArrangement({
      bed: FIORINO,
      boxes: [box({ stopSequence: 1 }), box({ stopSequence: 2 }), box({ count: 55, stopSequence: 3 })],
      payloadRatio: null,
    })
  ...
  test('cada recusa diz por que recusou', () => {
    ...
    expect(resolveStopArrangement({ bed: FIORINO, boxes: tresParadas(), payloadRatio: '0.6000' }).reason).toBe('weight')
    expect(
      resolveStopArrangement({
        bed: FIORINO,
        boxes: [box({ stopSequence: 1 })],
