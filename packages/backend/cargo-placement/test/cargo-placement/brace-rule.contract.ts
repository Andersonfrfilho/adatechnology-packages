...
import type { CargoPlacement, PlacedBox } from '../../src/cargo-placement.policy.js'
...
function isCenterUnsupported(boxes: readonly HandBox[]): boolean {
  return simulateUnloading(handBuilt(boxes), BED).unsupported.some(
    ({ box }) =>
      Math.abs(box.xM - CENTER.xM) < 1e-9 && Math.abs(box.yM - CENTER.yM) < 1e-9 && Math.abs(box.zM - CENTER.zM) < 1e-9,
  )
}
...
describe('a escora do juiz é encosto pelo lado (spec 135)', () => {
  ...
  test('a vizinha que só encosta 5 mm na face não escora', () => {
    ...
    expect(isCenterUnsupported([STEP, CENTER, back as HandBox, corner, left as HandBox, right as HandBox])).toBe(true)
  })
  ...
  test('a lâmina de 5 mm ao lado do cubo não sobe o bastante, e não escora', () => {
    ...
    expect(isCenterUnsupported([STEP, CENTER, back as HandBox, flat, left as HandBox, right as HandBox])).toBe(true)
  })
})
