...
describe('cargas reais de tamanhos misturados (spec 115)', () => {
  ...
  test('o Atego de 85 paradas desenha quase tudo, e só as primeiras entregas ficam fora', () => {
    const plan = place(ATEGO)
    const drawnStops = new Set(drawnOf(plan).map((box) => box.stopSequence))
    const missing = [...new Set(ATEGO.rows.map(([stop]) => stop))].filter((stop) => !drawnStops.has(stop))
  ...
  test('o desenho leva toda caixa empacotada, sem parada sumida nem caixa no ar', () => {
    ...
    expect(drawn.filter((box) => Math.abs(box.zM - supportUnder(box, drawn)) > SEAT_TOLERANCE_M)).toEqual([])
  })
})
