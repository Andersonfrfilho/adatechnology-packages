...
import { resolveCargoPlacement, type PlacedBox, type PlacementBox } from '../../src/cargo-placement.policy.js'
import { ACCELO_24_STOPS, ATEGO_85_STOPS, type RealCargoRow } from '../../src/fixtures/real-mixed-cargo.fixture.js'
...
function positionOf(box: PlacedBox) {
  return Object.fromEntries(Object.entries(box).filter(([key]) => key !== 'documentId' && key !== 'documentNumber'))
}
...
describe('cargo placement note identity contract (spec 119)', () => {
  ...
  test('publishes null when the note is unknown', () => {
    const boxes = placedOf(ACCELO_24_STOPS.slice(0, 20), false, ACCELO_BED).layers.flatMap((layer) => layer.boxes)
