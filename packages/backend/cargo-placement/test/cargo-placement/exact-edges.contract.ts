...
import type { LoadingAccess } from '../../src/loading-access.constant.js'
import {
  ACCELO_24_STOPS,
  ATEGO_85_STOPS,
  DAILY_19_STOPS,
...
function supportedFractionOf(box: PlacedBox, boxes: readonly PlacedBox[]): number {
  ...
  for (const other of boxes) {
    ...
    const acrossXM = Math.min(box.xM + box.depthM, other.xM + other.depthM) - Math.max(box.xM, other.xM)
    const acrossYM = Math.min(box.yM + box.widthM, other.yM + other.widthM) - Math.max(box.yM, other.yM)
  ...
  test.each([
    ...
    const boxes = drawnOf(
      resolveCargoPlacement({
        ...
        boxes: [boxOf({ count, heightMm: 200, lengthMm: sizeMm, stopSequence: 1, widthMm: sizeMm })],
        payloadRatio: null,
      }),
    )
    ...
    for (const box of boxes) expect(box.yM + box.widthM).toBeLessThanOrEqual(Number(widthM) + EPSILON)
  })
})
...
describe('as quatro viagens reais em coordenada exata (spec 132)', () => {
  ...
  test.each(REAL_LOADS.map((load) => [load.name, load] as const))(
    ...
    (_, load) => {
      ...
      const stopsOut = [...new Set(load.rows.map(([stop]) => stop))].filter((stop) => !drawnStops.has(stop))

      expect(boxes.length).toBeGreaterThanOrEqual(load.before.drawn)
      expect(boxes.filter((box) => !isComplement(box)).length).toBeGreaterThanOrEqual(load.before.recommended)
  ...
  test.each(REAL_LOADS.map((load) => [load.name, load] as const))(
    ...
    (_, load) => {
      const boxes = drawnOf(placeReal(load))
      const [lengthM, widthM, heightM] = [load.bed.lengthM, load.bed.widthM, load.bed.heightM].map(Number)
      ...
      expect(boxes.filter((box) => supportedFractionOf(box, boxes) < MIN_SUPPORTED_BASE_FRACTION - 1e-3)).toEqual([])
    },
  )
  ...
  test.each(REAL_LOADS.map((load) => [load.name, load] as const))(
    ...
    (_, load) => {
      ...
      expect(simulateUnloading({ ...plan, layers: [{ boxes: recommended, heightM: 0, index: 0 }] }, bed).stuck).toEqual(
        [],
      )
    },
  )
