  ...
  for (const load of REAL_LOADS) {
    ...
    test(`${load.name}: a ordem só é furada por caixa marcada para remanejar`, () => {
      ...
      for (const [earlier, later] of broken) {
        expect(earlier.reasons.includes('needsRehandling') || later.reasons.includes('needsRehandling')).toBe(true)
      }
    })
  }
})

function noteBox(input: Readonly<{ documentId: string; xM: number; yM?: number; zM?: number }>): PlacedBox {
  return {
    depthM: 0.261,
...
describe('a nota inteira junta (spec 120)', () => {
  ...
  test('conta pedaços por contato de face, com o vão da célula', () => {
    ...
    const stacked = [noteBox({ documentId: 'b', xM: 0 }), noteBox({ documentId: 'b', xM: 0, zM: 0.21 })]
    const apart = [noteBox({ documentId: 'c', xM: 0 }), noteBox({ documentId: 'c', xM: 0.5 })]
    const corner = [noteBox({ documentId: 'd', xM: 0 }), noteBox({ documentId: 'd', xM: 0.3, yM: 0.4 })]
