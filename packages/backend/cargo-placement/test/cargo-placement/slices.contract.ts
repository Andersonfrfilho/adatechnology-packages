...
describe('o equilíbrio longitudinal (spec 098)', () => {
  const cargo = [box({ count: 12, label: 'P1', stopSequence: 1 }), box({ count: 12, label: 'P2', stopSequence: 2 })]
...
describe('as faixas paralelas à porta (spec 100)', () => {
  ...
  test('carga pesada volta ao arranjo em profundidade', () => {
    const boxes = placed(resolveCargoPlacement({ bed: FIORINO, boxes: TRES_PARADAS, payloadRatio: '0.7000' }))
    /**
