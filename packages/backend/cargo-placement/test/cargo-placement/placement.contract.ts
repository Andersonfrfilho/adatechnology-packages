...
describe('empacotamento da carga (spec 094)', () => {
   ...
   *
   * ⚠️ Ela é **baixa** de propósito: o que bloqueia a faixa é a largura, e o volume precisa ser
   * desprezível para não mover as proporções que as outras afirmações desta suíte medem.
   */
  test('carrega a última parada no fundo e a primeira na porta', () => {
    const plan = resolveCargoPlacement({
      ...
      boxes: [
        box({ stopSequence: 1 }),
        box({ stopSequence: 3 }),
        box({ heightMm: 50, lengthMm: 2_200, stopSequence: 3, widthMm: 2_200 }),
      ],
    })
...
describe('caixa presumida (spec 094 P2)', () => {
  ...
  test('deriva a caixa do volume, na proporção da mediana medida', () => {
    ...
    const volume = ((fallback?.heightMm ?? 0) * (fallback?.lengthMm ?? 0) * (fallback?.widthMm ?? 0)) / 1e9
...
describe('desempenho do empacotador (spec 094 RF-NF)', () => {
  ...
  test('desenha toda caixa que cabe, sem teto de desenho', () => {
     ...
     */
    const boxes = Array.from({ length: 500 }, () => box({ count: 4, heightMm: 200, lengthMm: 200, widthMm: 200 }))
    const plan = resolveCargoPlacement({ bed: BED, boxes })
