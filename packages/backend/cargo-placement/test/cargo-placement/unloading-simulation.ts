 ...
 * uma vez só.
 */
function findUnsupportedBoxes(boxes: readonly PlacedBox[], bed: UnloadingBed): readonly UnsupportedBox[] {
  const unsupported: UnsupportedBox[] = []
  const present: PlacedBox[] = []
  const sequences = [...new Set(boxes.map((box) => box.stopSequence))].sort((first, second) => second - first)
...
function isBraced(input: Readonly<{ bed: UnloadingBed; box: PlacedBox; present: readonly PlacedBox[] }>): boolean {
  const { bed, box } = input
  ...
  return faces.every((face) => face.againstWall || isFaceBraced({ ...input, catchGapM, face, restraintM }))
}
...
function braceSpanOf(
  ...
  const reachesOut =
    face.sign > 0
      ? acrossToM > face.faceM + CONTACT_TOLERANCE_M && acrossFromM < face.faceM + input.catchGapM - CONTACT_TOLERANCE_M
      : acrossFromM < face.faceM - CONTACT_TOLERANCE_M && acrossToM > face.faceM - input.catchGapM + CONTACT_TOLERANCE_M
  ...
  return alongToM - alongFromM >= MIN_BRACE_CONTACT_M - CONTACT_TOLERANCE_M ? [alongFromM, alongToM] : null
}
...
function findStuckDeliveries(
  ...
  const personCells = Math.max(1, Math.round((options.personWidthM ?? PERSON_WIDTH_M) / ACCESS_CELL_M))
  ...
  const sequences = [...new Set(boxes.map((box) => box.stopSequence))].sort((first, second) => first - second)
  for (const stopSequence of sequences) {
...
function isWithinReach(
  ...
  for (let column = fromColumn - reach; column < toColumn + reach; column += 1) {
    for (let line = fromLine - reach; line < toLine + reach; line += 1) {
      ...
      const acrossColumns = column < fromColumn ? fromColumn - column - 1 : column >= toColumn ? column - toColumn : 0
