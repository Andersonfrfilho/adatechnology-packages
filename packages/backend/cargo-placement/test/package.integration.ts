...
async function createTemporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'cargo-placement-package-'))
...
test('packs a clean ESM package consumable by Bun without runtime dependencies', async () => {
  ...
  const archiveEntries = archiveContents.trim().split('\n').sort()
  const sharedTypeChunks = archiveEntries.filter((entry) => /^package\/dist\/[^/]+-[^/]+\.d\.ts$/.test(entry))
  expect(archiveEntries.filter((entry) => !sharedTypeChunks.includes(entry))).toEqual([
    'package/README.md',
  ...
  await Bun.write(
    ...
    JSON.stringify({ name: 'cargo-placement-consumer', private: true, type: 'module' }),
  )
  await Bun.write(
    join(consumerDirectory, 'verify.ts'),
    [
      "import { resolveCargoPlacement, LOADING_ACCESS_KINDS } from '@adatechnology/cargo-placement'",
      "import type { PlacementBox, ResolvedCargoLayout } from '@adatechnology/cargo-placement'",
      "import { MIXED_BEDS } from '@adatechnology/cargo-placement/fixtures'",
      '',
      'const layout: ResolvedCargoLayout | undefined = undefined',
      "const box: PlacementBox = { count: 1, heightMm: 100, isFragile: false, isStackable: true, keepUpright: false, label: 'Parada 1', lengthMm: 100, maxStackCount: null, source: 'measured', stopSequence: 1, widthMm: 100 }",
      "const placement = resolveCargoPlacement({ bed: { heightM: '2.000', lengthM: '4.000', source: 'measured', widthM: '2.000' }, boxes: [box], loadingAccess: LOADING_ACCESS_KINDS[0], payloadRatio: null })",
      "if (placement === null || placement.layers.length !== 1 || MIXED_BEDS.length === 0 || layout !== undefined) throw new Error('invalid package exports')",
    ].join('\n'),
  )
  await Bun.write(
  ...
  const installedPackageJson = await readFile(
    join(consumerDirectory, 'node_modules/@adatechnology/cargo-placement/package.json'),
    'utf8',
  )
  const installedModule = await readFile(
    join(consumerDirectory, 'node_modules/@adatechnology/cargo-placement/dist/index.js'),
    'utf8',
  )
