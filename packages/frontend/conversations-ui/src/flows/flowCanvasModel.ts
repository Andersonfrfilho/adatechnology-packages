 ...
 *
 * Derivações puras do canvas do editor: posições, arestas, contagem de conversas ao vivo e o nó
 * que nasce de cada item da paleta.
 *
 * Separadas do componente porque é aqui que mora o risco. Uma aresta com destino errado não pinta
 * errado — ela leva a conversa do cliente para o lugar errado, e o sintoma aparece longe de quem
 * editou. Cor de traço, por outro lado, ninguém quebra sem ver. Então a topologia sai daqui em
 * forma neutra (`FlowEdgeSpec`) e o estilo fica no componente: o teste cobra o que importa e roda
 * sem navegador nem `@xyflow/react`.
 */
...
/** Onde uma conversa está agora. O produto normaliza — o pacote não sabe o que é "estado". */
...
export function countLiveByNode(
  flowKey: string,
  positions: readonly FlowLivePosition[] | undefined,
): Record<string, number> {
  const counts: Record<string, number> = {}

  for (const position of positions ?? []) {
    if (position.flowKey !== flowKey || !position.nodeId) continue
    counts[position.nodeId] = (counts[position.nodeId] ?? 0) + 1
  }

  return counts
}

type MergedLayoutParams = {
 ...
 *
 * Posicionar cada fluxo à parte e deslocar não resolve: nada impede dois fluxos de ocuparem o
 * mesmo espaço, e a altura real de cada card é ignorada. Aqui o ranqueamento por BFS roda sobre o
 * grafo mesclado inteiro, com os saltos `flow:<key>` já resolvidos para o nó inicial do alvo.
 */
export function computeMergedLayout(params: MergedLayoutParams): Map<string, FlowNodePosition> {
  ...
  // Todo nó não alcançado vai para UMA camada extra abaixo de tudo. Uma camada por órfão empurrava
  // cada um para uma linha própria, cada vez mais longe da área visível — e desligar um fio se lia
  // como "apaguei o card".
...
export type FlowEdgeSpec = {
  ...
  /** Salto entre fluxos — desenhado tracejado, esteja o alvo mesclado ou num portal. */
  readonly crossFlow: boolean
  readonly live: boolean
}

type BuildEdgesParams = {
  readonly openKeys: readonly string[]
  readonly graphs: Readonly<Record<string, FlowGraphData>>
 ...
 * ligados mostrava uma caixa de portal no meio de dois cards que já estavam ali do lado.
 */
export function buildFlowEdges(params: BuildEdgesParams): FlowEdgeSpec[] {
  const { openKeys, graphs, livePositions } = params
  const open = new Set(openKeys)
  ...
  for (const flowKey of openKeys) {
    ...
    for (const [nodeId, node] of Object.entries(graph.nodes)) {
      ...
      for (const { target, optionId, isDefault } of targetsOf(node)) {
        ...
        // `targetsOf` devolve o `default` mesmo em branco. Emitir a aresta criaria ligação para um
        // nó que não existe — o React Flow a descarta em silêncio, então o sintoma seria uma opção
        // que parece ligada e não está.
        if (!target) continue
        ...
        if (isDefault) {
          edges.push({
            ...
            kind: 'fallback',
            crossFlow,
            // Fallback não anima: ele é o caminho que ninguém escolheu, e piscar dava a impressão
            // de que a conversa estava passando por ali.
            live: false,
          })
          continue
        }
...
export function chainFrameBounds(params: {
  ...
  // O topo abre espaço extra para o rótulo da moldura, que é desenhado acima da borda.
  const labelRoom = 24
  return {
    x: minX - padding,
    y: minY - padding - labelRoom,
    width: maxX - minX + padding * 2,
    height: maxY - minY + padding * 2 + labelRoom,
  }
}
 ...
 *
 * `contextKey` igual ao id porque é o que o motor usa para guardar a resposta, e deixá-lo vazio
 * fazia a pergunta ser feita e a resposta descartada — sem erro em lugar nenhum.
 */
