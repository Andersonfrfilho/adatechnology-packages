...
import { isValidCnpj } from './checkDigit.service'
import { readValueBelowLabel } from './labelGeometry.service'
import type { PdfPageText } from './pdfTextLayer.service'

/**
 * O mapa de rótulos do CCMEI mora aqui, e não no app, porque **duas** apps o leem: a landing, no
 * navegador de quem anexa, e a API, no arquivo que chegou ao bucket. O critério é esse — o mapa do
 * CRLV continua no painel justamente porque só o painel lê CRLV.
 *
 * O que **não** vem junto é o que decide o que fazer com a leitura: encaixar no formulário e
 * comparar com o que a pessoa digitou são assunto de quem tem formulário. A biblioteca extrai.
 *
...
function readAddress(input: { readonly collected: Collected; readonly page: PdfPageText }): CcmeiAddress | undefined {
