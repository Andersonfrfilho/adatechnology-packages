 ...
 * com camada de texto de verdade: prova bytes → fragmento → geometria → campo. O que ele não prova é
 * que o layout do Detran é este — isso é conferência manual, e está escrito no `evidence.md`.
 */
...
export const VALID_CRLV_RENAVAM = '00123456789'
export const VALID_CRLV_CPF = '111.444.777-35'

/** O CRLV de referência da spec: três colunas de rótulo e valor, título no topo e rodapé da CDT. */
export function buildCrlvPdf(
  ...
  const printed: Readonly<Record<string, string>> = {
    'ANO MODELO': '2021',
    CARROCERIA: 'FURGAO',
    'CODIGO RENAVAM': VALID_CRLV_RENAVAM,
    COMBUSTIVEL: 'ALCOOL/GASOLINA',
    'COR PREDOMINANTE': 'BRANCA',
    'CPF / CNPJ': VALID_CRLV_CPF,
    EIXOS: '2',
  ...
  return buildTextPdf([CRLV_TITLE_PLACEMENT, ...buildLabelledColumns(columns), CDT_FOOTER_PLACEMENT, ...extra])
}
