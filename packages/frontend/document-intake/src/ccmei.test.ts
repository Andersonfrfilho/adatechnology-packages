...
import { readCcmei } from './ccmei.service'

import { buildLabelledColumns, buildTextPdf } from './pdfFixture.helper'
...
async function readCcmeiPdf(columns: readonly Readonly<{ label: string; value: string; x: number; y: number }>[]) {
...
describe('leitura do CCMEI', () => {
  ...
  test('a data sai em ISO, que é o formato com que ela vai ser comparada', async () => {
    const reading = await readCcmeiPdf([{ label: 'Data de Início de Atividades', value: '01/12/2020', x: 60, y: 600 }])
  ...
  test('CNPJ que não fecha o dígito verificador não vira valor', async () => {
    const reading = await readCcmeiPdf([{ label: 'CNPJ', value: '30.213.061/0001-07', x: 60, y: 600 }])
  ...
  test('data impossível não vira valor, e diz por quê', async () => {
    const reading = await readCcmeiPdf([{ label: 'Data de Início de Atividades', value: '31/02/2018', x: 60, y: 600 }])
  ...
  test('rótulo ausente vira ressalva, não campo em branco', async () => {
    const reading = await readCcmeiPdf([{ label: 'CNPJ', value: '30.213.061/0001-06', x: 60, y: 600 }])
  ...
  test('aceita CNPJ alfanumérico', async () => {
    const reading = await readCcmeiPdf([{ label: 'CNPJ', value: '12.ABC.345/01DE-35', x: 60, y: 600 }])
